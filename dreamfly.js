...
function uploadImage(img, tab, fName, imgType) {
	...
	xhr.open('POST', 'https://search.jd.com/image?op=upload', true);
	...
	xhr.onload = function (e) {
		if (xhr.readyState === 4 && xhr.status === 200) {
			var str = xhr.response.match(/(?<=parent.\$o.uploader.callback\(').*(?='\))/);
			if(str==null){
				var r=confirm("您还没有登录京东，点击确定跳转京东登录页面");
			    if (r==true){
     				chrome.tabs.create({
				...
				var err = str.toString().split(".")[1];
				switch (err) {
			        case "UPLOAD_RETRY":
			            alert("抱歉，图片上传失败, 网络状况不好，请重新上传试试~");
			            break;
			        case "UPLOAD_SIZE":
			            alert("抱歉，图片上传失败, 图片不能大于4M，请换一张图试试~");
			            break;
			        case "UPLOAD_MIN_WH":
			            alert("抱歉，图片上传失败, 图片尺寸不能小于201x201 px");
			            break;
			        case "UPLOAD_FORMAT":
			            alert("抱歉，图片上传失败, 只支持jpg或png格式的图片");
			            break;
			        case "UPLOAD_JSF":
			            alert("抱歉，图片上传失败, 网络状况不好，请重新上传试试~");
			            break;
	                default:
	                    var url = "https://search.jd.com/image?path=" + encodeURIComponent(str) + "&op=search"
	     				chrome.tabs.create({
							url : url
						});
		        }
...
chrome.webRequest.onBeforeSendHeaders.addListener(function (details) {
    ...
    {
        urls: ["https://www.jd.com/*","https://search.jd.com/*"]
    },
